...
path = "/home/felix/fklose/Data/ROOT_Files/"
fname = "output03674.root"
...
ax[0].legend()
ax[1].legend()
fig.suptitle("TOF_LE Cut (TDC_ION_MCP_LE - TDC_PHOTO_DIODE_LE)")
plt.close()

# Optical Pumping time (OP)
TTTL_OP_Beam_Cut = (0.25 <= TTTL_OP_Beam) & (TTTL_OP_Beam <= 1.16)
# TTTL_OP_Beam_Cut = (0 <= TTTL_OP_Beam)
...
ax[0].set_xlabel("X1-X2 [$ns$]")
ax[1].set_xlabel("X1-X2 [$ns$]")
ax[0].set_ylabel("Z1-Z2 [$ns$]")
ax[1].set_ylabel("Z1-Z2 [$ns$]")
plt.close()
...
plt.errorbar(x, y, np.sqrt(y), ls="", color="black", marker=".")
plt.xlabel("Frequency wrt $^{39}$K cog [MHz]")
plt.ylabel("Counts")
plt.show()