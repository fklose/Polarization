...
# Get a better guess using least squares fit
p1, _ = curve_fit(model, x, n, p0, bounds=([0, 0, 0, 0, 0, 0], [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf]), max_nfev=1000)
p0 = p1
# Perform final fit using a poisson fit
# p, E1, E2, sigma, X2 = poisson_fit(model, x, n, p, iter=200)
p, E1, E2, sigma, X2 = alt_poisson_fit(model, x, n, p0, bounds=[(0, np.inf)]*len(p0))
